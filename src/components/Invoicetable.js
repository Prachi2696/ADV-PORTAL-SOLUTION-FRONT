...
export default function Invoicetable() {
    ...
    //     const fetchData = () => {
    //         // setSpinner(true)
    //         fetch(BaseurlAdmin + "igniteinvoice/OMC002", {
    //             method: "post",
    //             // body: decryptedText,
    //             headers: {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(BaseurlAdmin + "igniteinvoice/OMC002", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Access-Control-Allow-Origin": "*",
            ...
            <Grid container spacing={1}>
                <Grid item xs={2} lg={2}>

                    {
                        list.map((item, index) =>
                            <li key={index}>
                                <ul> {item.client_id}</ul>
                                <ul>   {item.invoice_no}</ul>
